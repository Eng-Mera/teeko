...
function dt_theme_mailchimp_subscribe() {
	...
	if($apiKey != '' && $listId != '') {
		...
		if(dt_theme_mailchimp_check_member_already_registered($data, $apiKey, $listId)) {
			$out = '<span class="error-msg"><b>'.esc_html__('Error:', 'kidsworld-core').'</b> '.esc_html__('You have already subscribed with us !', 'kidsworld-core').'</span>';
		} else {
			$out = dt_theme_mailchimp_register_member($data, $apiKey, $listId);
		}

	} else {
		$out = '<span class="error-msg"><b>'.esc_html__('Error:', 'kidsworld-core').'</b> '.esc_html__('Please make sure valid mailchimp details are provided.', 'kidsworld-core').'</span>';
	}

	echo kidsworld_wp_kses($out);
...
function dt_theme_mailchimp_register_member($data, $apiKey, $listId) {
	...
	if($httpCode == 200) {
		$out = '<span class="success-msg">'.esc_html__('Success! Please check your inbox or spam folder.', 'kidsworld-core').'</span>';
	} else {
