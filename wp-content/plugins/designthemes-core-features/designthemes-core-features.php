...
if (! class_exists ( 'DTCorePlugin' )) {
	...
	class DTCorePlugin {
		...
		function dtLoadPluginTextDomain() {
			load_plugin_textdomain ( 'kidsworld-core', false, dirname ( plugin_basename ( __FILE__ ) ) . '/languages/' );
		}

		/**
		 */
		public static function dtCorePluginActivate() {
			if( ! function_exists('kidsworld_option') ){
				wp_die( esc_html__( 'Please make sure "KidsWorld Theme" is activated.', 'kidsworld-core' ) );
			}
		}
