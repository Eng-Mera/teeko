...
class DTCoreShortcodesDefination {
	...
	function dt_sc_pricing_table_item($attrs, $content = null) {
		...
		$class = '';
		if($color != '') {
			$unique_class = dttheme_generate_random_number('pricingtable');
			$css = dttheme_generate_pricingtable_css( $unique_class, $attrs );
			dttheme_generate_custom_css( $css );
			$class = $unique_class;
		}

		$selected = ( strtolower($highlight) == "yes" ) ? 'selected' : '';

		$out = "<div class='dt-sc-pr-tb-col type1 $selected $class'>";
	...
	function dt_sc_pricing_table_minimal_item($attrs, $content = null) {
		...
		if($color != '') {
			$unique_class = dttheme_generate_random_number('pricingtable');
			$css = dttheme_generate_pricingtable_minimal_css( $unique_class, $attrs );
			dttheme_generate_custom_css( $css );
			$class = $unique_class;
		}
		...
		$out = "<div class='dt-sc-pr-tb-col minimal $selected $class'>";
	...
	function dt_sc_pricing_table_type2_item($attrs, $content = null) {
		...
		if($color != '') {
			$unique_class = dttheme_generate_random_number('pricingtable');
			$css = dttheme_generate_pricingtable_type2_css( $unique_class, $attrs );
			dttheme_generate_custom_css( $css );
			$class = $unique_class;
		}
		...
		$out = "<div class='dt-sc-pr-tb-col type2 $selected $class'>";
			$out .= '<div class="dt-sc-tb-header">';
				$out .= '<div class="dt-sc-tb-title">';
	...
	function dt_sc_iconbox($attrs, $content = null) {
		...
		$class .= !empty($variations) ? ' '.$variations : '';

		if($color != '') {
			$unique_class = dttheme_generate_random_number('iconbox');
			$css = dttheme_generate_iconbox_css( $unique_class, $attrs );
			dttheme_generate_custom_css( $css );
			$class .= ' '.$unique_class;
		}
		...
		$out = "<div class='dt-sc-icon-box {$type} {$class}'{$style}>";
			...
			$out .= '<div class="icon-content">';
				if($type == 'type2')
					$out .= '<span>'.strtoupper(substr($title, 0, 1)).'</span>';
				if(!empty($subtitle))
					$out .= "<h5>{$subtitle}</h5>";
	...
	function dt_sc_blockquote($attrs, $content = null) {
		...
		$title = ($type == 'type4' && !empty($title)) ? '<h3>'.$title.'</h3>' : '';

		$class = join( '', array_unique( $class ) );

		$out = "<blockquote class='{$type} {$class}'>{$title}{$content}{$cite}</blockquote>";
		return $out;
	}
	...
	function dt_sc_woo_cart($attrs, $content = null) {
		...
		if( function_exists("is_woocommerce")) :
		    $out = '<a class="cart-info" href="'.WC()->cart->get_cart_url().'" title="'.esc_attr__( 'View Shopping Cart', 'kidsworld-core' ).'">';
		        $count = WC()->cart->cart_contents_count;
		        $out .= '<p class="cart-icon"> <span> '.$count.' </span> </p>';
		        $out .= '<p class="cart-total"> '.WC()->cart->get_cart_total().' </p>';
		    $out .= '</a>';
		else:
			$out .= '<p>'.esc_html__('Kindly install & activate woocommerce plugin and check it.', 'kidsworld-core').'</p>';
		endif;
	...
	function dt_sc_contact_info($attrs, $content = null) {
		...
		if($color != '') {
			$unique_class = dttheme_generate_random_number('contactinfo');
			$css = dttheme_generate_contactinfo_css( $unique_class, $attrs );
			dttheme_generate_custom_css( $css );
			$class .= ' '.$unique_class;
		}
	...
	function dt_sc_image_caption($attrs, $content = null) {
		...
		if($color != '') {
			$unique_class = dttheme_generate_random_number('imagecaption');
			$css = dttheme_generate_imagecaption_css( $unique_class, $attrs );
			dttheme_generate_custom_css( $css );
			$class .= ' '.$unique_class;
		}
	...
	function dt_sc_mc_newsletter($attrs, $content = null) {
		extract ( shortcode_atts ( array (
			...
			'tooltip' => '',
			'listid' => '',
			'name' => esc_html__('Your Name', 'kidsworld-core'),
			'email' => esc_html__('Your Email Address', 'kidsworld-core'),
			'button' => esc_html__('Subscribe Now', 'kidsworld-core'),
			'show_name' => '',
			'class' => ''
		), $attrs ) );
		...
		$out = "<div class='dt-sc-newsletter-section {$type} {$class}'>";
			...
			if($type == 'type6') {
				$out .= '<div class="newsletter-content">';
			}
			...
			$mcapi = kidsworld_option('layout','mailchimp-key');
	...
	function dt_sc_twitter_tweets($attrs, $content = null) {
		...
		if($username && $consumerkey && $consumersecret && $accesstoken && $accesstokensecret) {
			...
			require_once(KIDSWORLD_CORE_PLUGIN."/apis/twitteroauth/twitteroauth.php");
			...
			$out .= '<div class="dt-sc-twitter-feeds">';
				...
				if($twitter && is_array($twitter)) {
					...
					$out .= '<div class="no-tweets">'.esc_html__('No Twitter Tweets found.','kidsworld-core').'</div>';
				}
			$out .= '</div>';
		} else {
			$out .= '<div class="no-tweets">'.esc_html__('Invalid Twitter Authentication keys.','kidsworld-core').'</div>';
		}
	...
	function dt_sc_post( $attrs, $content = null ){
		...
		$class = '';
		if($color != '') {
			$unique_class = dttheme_generate_random_number('post');
			$css = dttheme_generate_post_css( $unique_class, $attrs );
			dttheme_generate_custom_css( $css );
			$class = $unique_class;
		}
		...
		if( $the_query->have_posts() ):
			while( $the_query->have_posts() ):
				...
				$out .= '<article id="post-'.esc_attr($id).'" class="'.esc_attr($post_classes).' '.esc_attr($class).'">';
					#Featured Image
					...
					if( $style == "entry-date-left") :
						...
						$out .= '		<div class="comments '.esc_attr($hide_comment_meta).'">';
											...
											$out .= '	<i class="fa fa-comment"></i>'.esc_html($commenttext);
											$out .= '</a>';
						$out .= '		</div>';
						$out .= '	</div>';
						...
						$out .= '	</div>';
									...
									if( strtolower($allow_excerpt) == 'yes' ):
										$out .= '<div class="entry-body">'.kidsworld_excerpt($excerpt_length).'</div>';
									endif;
					...
					elseif( $style == "default"):
						...
						$tclass = ( ($hide_comment_meta == "hidden" ) && ($hide_author_meta == "hidden" ) ) ? "hidden" : "";
						$out .= '<div class="entry-details">';
						$out .= '	<div class="entry-meta '.esc_attr($tclass).'">';

						$out .= '	<div class="date '.esc_attr($hide_date_meta).'">'.esc_html__('Posted on','kids-world').' '.get_the_date(get_option('date_format')).'</div>';
						...
						$out .= 			esc_html__('Posted on','kidsworld-core').get_the_date(' d M Y');
						$out .= '		</div>';
						$out .= '		<div class="comments '.esc_attr($hide_comment_meta).'">';
											$commenttext = "";
											if((wp_count_comments($id)->approved) == 0):
											...
											$out .= '/ <a class="comments" href="'.esc_url($link).'/#respond">';
											$out .= '	<i class="fa fa-comment"></i> '.esc_html($commenttext).' '.esc_html__('Comments','kidsworld-core');
											$out .= '</a>';
						$out .= '		</div>';
						$out .= '		<div class="author '.esc_attr( $hide_author_meta ).'">';
						$out .= '			/ <i class="fa fa-user"> </i>';
	...
	function dt_sc_recent_post( $attrs, $content = null ){
		...
		$warning = esc_html__('No Posts Found','kidsworld-core');
		
		if( !empty($category) ){
			$args = array( 'posts_per_page' => $count, 'orderby' => 'date', 'cat' => $category );
			$warning = esc_html__('No Posts Found in Category ','kidsworld-core').$category;
		}
		...
		if($the_query->have_posts()): $i = 1;
		 while($the_query->have_posts()): $the_query->the_post();
			...
			$out .= "<div class='blog-entry blog-thumb-style entry-date-left {$lastcls}'>";
				$out .= '<div class="entry-thumb">';
					$out .= get_the_post_thumbnail( get_the_ID(), 'kidsworld-blog-thumb' );
				$out .= '</div>';
				$out .= '<div class="entry-details">';
	...
	function dt_sc_blog_related_post( $attrs, $content = null ){
		...
		$hide_post_format = kidsworld_option('pageoptions','post-format-meta'); 
		$hide_post_format = isset( $hide_post_format )? "yes" : "";
	
		$hide_author_meta = kidsworld_option('pageoptions','post-author-meta');
		$hide_author_meta = isset( $hide_author_meta ) ? "yes" : "";
	
		$hide_date_meta = kidsworld_option('pageoptions','post-date-meta');
		$hide_date_meta = isset( $hide_date_meta ) ? "yes" : "";	

		$hide_comment_meta = kidsworld_option('pageoptions','post-comment-meta');
		$hide_comment_meta = isset( $hide_comment_meta ) ? "yes" : "";

		$hide_category_meta = kidsworld_option('pageoptions','post-category-meta');
		$hide_category_meta = isset( $hide_category_meta ) ? "yes" : "";
	
		$hide_tag_meta = kidsworld_option('pageoptions','post-tag-meta');
		...
		$excerpt_length = kidsworld_option('pageoptions','post-archives-excerpt');
		...
		if( $the_query->have_posts() ) {
			...
			$out .= 	'<h4>'.esc_html__('Related posts','kidsworld-core').'</h4>';
	...
	function dt_sc_portfolios( $attrs, $content = null ){
		...
		if( (sizeof($categories) > 1) && ( $allow_filter == "yes" ) ) :
			...
			$out .= '	<a href="#" class="active-sort" title="" data-filter=".all-sort">'.esc_html__('All','kidsworld-core').'</a>';
		...
		if($the_query->have_posts()) :
			...
			if( $ajax_call == 'no' ) :
				...
				if( $display_style == 'load-more' || $display_style == 'lazy' ) :
					$label = ( $display_style == 'load-more' ) ? esc_html__('Load More', 'kidsworld-core' ) : esc_html__('Scroll To Load More','kidsworld-core');
	...
	function dt_sc_events_list( $attrs, $content = null ){
		...
		$out = '<div class="dt-sc-events-list-wrapper">';
			...
			if($events):							
				...
				$out .= '<div class="dt-sc-warning-box">'.esc_html__('No Events Found','kidsworld-core').'</div>';
			endif;	
			wp_reset_postdata();
	...
	function dt_sc_special_events_list( $attrs, $content = null ){
		...
		$class = '';
		if($color != '') {
			$unique_class = dttheme_generate_random_number('events');
			$css = dttheme_generate_events_css( $unique_class, $attrs );
			dttheme_generate_custom_css( $css );
			$class = $unique_class;
		}		
			...
			switch($type):
				...
				default:
					foreach($events as $post):
						...
						$out .= '<div class="dt-sc-one-third column '.$temp_class.'">';
							$out .= '<div class="dt-sc-event type1 '.$class.'">';
								 ...
								 $out .= '<div class="dt-sc-event-thumb">';
									if(has_post_thumbnail()):
										...
										$out .= get_the_post_thumbnail($post->ID, 'kidsworld-event-list2', $attr);
									else:
				...
				case 'type2':
					foreach($events as $post):
						...
						$out .= '<div class="dt-sc-one-third column '.$temp_class.'">';
							$out .= '<div class="dt-sc-event type2 '.$class.'">';
								 $title = ( strlen(get_the_title()) > 27 ) ? substr(get_the_title(),0,27)."..." : get_the_title();
								 $out .= '<h2 class="entry-title"><a href="'.get_permalink().'">'.$title.'</a></h2>';
								 $out .= '<div class="dt-sc-event-thumb">';
									if(has_post_thumbnail()):
				...
				case 'type3':
					foreach($events as $post):
						...
						$out .= '<div class="dt-sc-one-third column '.$temp_class.'">';
							$out .= '<div class="dt-sc-event type3 '.$class.'">';
								$out .= '<div class="dt-sc-event-thumb">';
									if(has_post_thumbnail()):
										$attr = array('title' => get_the_title(), 'alt' => get_the_title());
										$out .= get_the_post_thumbnail($post->ID, 'kidsworld-event-list2', $attr);
				...
				case 'type4':
					foreach($events as $post):
						...
						$out .= '<div class="dt-sc-one-third column '.$temp_class.'">';
							$out .= '<div class="dt-sc-event type4 '.$class.'">';
								$out .= '<div class="dt-sc-event-thumb">';
									if(has_post_thumbnail()):
										$attr = array('title' => get_the_title(), 'alt' => get_the_title());
										$out .= get_the_post_thumbnail($post->ID, 'kidsworld-event-list2', $attr);
				...
				case 'type5':
					foreach($events as $post):
						...
						//$out .= '<div class="dt-sc-one-half column '.$temp_class.'">';
							$out .= '<div class="dt-sc-event type5 '.$class.'">';
								...
								//$out .= '<div class="dt-sc-one-half column">';
									$out .= '<div class="events-details">';
										$title = ( strlen(get_the_title()) > 18 ) ? substr(get_the_title(),0,18)."..." : get_the_title();
										$out .= '<h2 class="entry-title"><a href="'.get_permalink().'">'.$title.'</a></h2>';
										$out .= '<div class="dt-sc-event-meta">';
											$out .= '<p class="dt-sc-event-time">';
												$out .= '<span><i class="fa fa-clock-o"></i>'.tribe_get_start_time ( $event_id, 'h:i a' ).' - '.tribe_get_end_time ( $event_id, 'h:i a' ).'</span>';
											$out .= '</p>';
											$out .= '<p class="dt-sc-event-venue">'.tribe_get_venue($event_id).'</p>';
										$out .= '</div>';
									$out .= '</div>';
									$out .= '<div class="events-cost-booking">';
										$out .= '<div class="dt-sc-event-cost">';
													$ecost = tribe_get_formatted_cost($event_id);
													if(!empty($ecost)) {
														$out .= esc_html__('from', 'kidsworld-core').' '.'<span class="event-booking-price">'.$ecost.'</span>';
													}
										$out .= '</div>';
										$out .= '<a href="'.get_permalink().'" class="dt-sc-events-read-more">'.esc_html__('Find out more', 'kidsworld-core').'</a>';
									$out .= '</div>';
								//$out .= '</div>';
							$out .= '</div>';
						//$out .= '</div>';
	...
	function dt_sc_down_count($attrs, $content = null) {
		...
		if( !kidsworld_option('pageoptions','show-launchdate') ) return;

		$out .= '<div class="downcount '.$class.'" data-date="'.kidsworld_opts_get('comingsoon-launchdate', date('m/d/Y')).'" data-offset="'.kidsworld_opts_get('comingsoon-timezone', '0').'">';
			$out .= '<div class="dt-sc-counter-wrapper">';
				$out .= '<div class="counter-icon-wrapper">';
					$out .= '<div class="dt-sc-counter-number days">00</div>';
				$out .= '</div>';
				if($type == 'type2')
					$out .= '<h3 class="title">'.esc_html__('Days Left', 'kidsworld-core').'</h3>';
				else
					$out .= '<h3 class="title">'.esc_html__('Days', 'kidsworld-core').'</h3>';
			$out .= '</div>';
			$out .= '<div class="dt-sc-counter-wrapper">';
				$out .= '<div class="counter-icon-wrapper">';
				...
				$out .= '</div>';
				if($type == 'type2')
					$out .= '<h3 class="title">'.esc_html__('Hr.', 'kidsworld-core').'</h3>';
				elseif($type == 'type5')
					$out .= '<h3 class="title">'.esc_html__('Hrs', 'kidsworld-core').'</h3>';
				else
					$out .= '<h3 class="title">'.esc_html__('Hours', 'kidsworld-core').'</h3>';
			$out .= '</div>';
			$out .= '<div class="dt-sc-counter-wrapper">';
				$out .= '<div class="counter-icon-wrapper">';
				...
				$out .= '</div>';
				if($type == 'type2')
					$out .= '<h3 class="title">'.esc_html__('Min.', 'kidsworld-core').'</h3>';
				elseif($type == 'type5')
					$out .= '<h3 class="title">'.esc_html__('Mins', 'kidsworld-core').'</h3>';
				else
					$out .= '<h3 class="title">'.esc_html__('Minutes', 'kidsworld-core').'</h3>';
			$out .= '</div>';
			$out .= '<div class="dt-sc-counter-wrapper last">';
				...
				$out .= '</div>';
				if($type == 'type2')
					$out .= '<h3 class="title">'.esc_html__('Sec.', 'kidsworld-core').'</h3>';
				elseif($type == 'type5')
					$out .= '<h3 class="title">'.esc_html__('Secs', 'kidsworld-core').'</h3>';
				else
					$out .= '<h3 class="title">'.esc_html__('Seconds', 'kidsworld-core').'</h3>';
			$out .= '</div>';
		$out .= '</div>';
	...
	function dt_sc_sociable($attrs, $content = null) {
		...
		$socials = kidsworld_option('social');
		$out = "";
	...
	function dt_sc_ad_carousel_wrapper( $atts, $content = null ) {
		...
		return '<div class="carousel_items '.$class.'">'
					...
					.'<div class="dt-carousel-navigation">'
