	...
	<div class="page" id="register-page">
		...
		<?php if ( 'registration-disabled' == bp_get_current_signup_step() ) : ?>
			...
			do_action( 'bp_before_registration_disabled' ); ?>
				...
				<p><?php _e( 'User registration is currently not allowed.', 'kids-world' ); ?></p>
		...
		<?php if ( 'request-details' == bp_get_current_signup_step() ) : ?>
			...
			<p><?php _e( 'Registering for this site is easy. Just fill in the fields below, and we\'ll get a new account set up for you in no time.', 'kids-world' ); ?></p>
			...
			<div class="register-section" id="basic-details-section">
				...
				<h2><?php _e( 'Account Details sdfsdfdsf', 'kids-world' ); ?></h2>

				<label for="signup_username"><?php _e( 'Username', 'kids-world' ); ?> <?php _e( '(required)', 'kids-world' ); ?></label>
				<?php
				...
				<label for="signup_email"><?php _e( 'Email Address', 'kids-world' ); ?> <?php _e( '(required)', 'kids-world' ); ?></label>
				<?php
				...
				<label for="signup_password"><?php _e( 'Choose a Password', 'kids-world' ); ?> <?php _e( '(required)', 'kids-world' ); ?></label>
				<?php
				...
				<label for="signup_password_confirm"><?php _e( 'Confirm Password', 'kids-world' ); ?> <?php _e( '(required)', 'kids-world' ); ?></label>
				<?php
			...
			<?php if ( bp_is_active( 'xprofile' ) ) : ?>
				...
				<div class="register-section" id="profile-details-section">
					...
					<h2><?php _e( 'Profile Details', 'kids-world' ); ?></h2>
						...
						<div<?php bp_field_css_class( 'editfield' ); ?>>
							...
							if ( bp_current_user_can( 'bp_xprofile_change_field_visibility' ) ) : ?>
								<p class="field-visibility-settings-toggle" id="field-visibility-settings-toggle-<?php bp_the_profile_field_id() ?>">
									<?php
									printf(
										__( 'This field can be seen by: %s', 'kids-world' ),
										'<span class="current-visibility-level">' . bp_get_the_profile_field_visibility_level_label() . '</span>'
									);
									?>
									<button type="button" class="visibility-toggle-link"><?php _ex( 'Change', 'Change profile field visibility level', 'kids-world' ); ?></button>
								</p>

								<div class="field-visibility-settings" id="field-visibility-settings-<?php bp_the_profile_field_id() ?>">
									<fieldset>
										<legend><?php _e( 'Who can see this field?', 'kids-world' ) ?></legend>

										<?php bp_profile_visibility_radio_buttons() ?>

									</fieldset>
									<button type="button" class="field-visibility-settings-close"><?php _e( 'Close', 'kids-world' ) ?></button>
			...
			<?php if ( bp_get_blog_signup_allowed() ) : ?>
				...
				<div class="register-section" id="blog-details-section">
					...
					<h2><?php _e( 'Blog Details', 'kids-world' ); ?></h2>

					<p><label for="signup_with_blog"><input type="checkbox" name="signup_with_blog" id="signup_with_blog" value="1"<?php if ( (int) bp_get_signup_with_blog_value() ) : ?> checked="checked"<?php endif; ?> /> <?php _e( 'Yes, I\'d like to create a new site', 'kids-world' ); ?></label></p>

					<div id="blog-details"<?php if ( (int) bp_get_signup_with_blog_value() ) : ?>class="show"<?php endif; ?>>

						<label for="signup_blog_url"><?php _e( 'Blog URL', 'kids-world' ); ?> <?php _e( '(required)', 'kids-world' ); ?></label>
						<?php
						...
						<label for="signup_blog_title"><?php _e( 'Site Title', 'kids-world' ); ?> <?php _e( '(required)', 'kids-world' ); ?></label>
						<?php
						...
						<fieldset class="register-site">
							<legend class="label"><?php _e( 'Privacy: I would like my site to appear in search engines, and in public listings around this network.', 'kids-world' ); ?></legend>
							<?php
							...
							<label for="signup_blog_privacy_public"><input type="radio" name="signup_blog_privacy" id="signup_blog_privacy_public" value="public"<?php if ( 'public' == bp_get_signup_blog_privacy_value() || !bp_get_signup_blog_privacy_value() ) : ?> checked="checked"<?php endif; ?> /> <?php _e( 'Yes', 'kids-world' ); ?></label>
							<label for="signup_blog_privacy_private"><input type="radio" name="signup_blog_privacy" id="signup_blog_privacy_private" value="private"<?php if ( 'private' == bp_get_signup_blog_privacy_value() ) : ?> checked="checked"<?php endif; ?> /> <?php _e( 'No', 'kids-world' ); ?></label>
						</fieldset>
			...
			<div class="submit">
				<input type="submit" name="signup_submit" id="signup_submit" value="<?php esc_attr_e( 'Complete Sign Up', 'kids-world' ); ?>" />
			</div>
		...
		<?php if ( 'completed-confirmation' == bp_get_current_signup_step() ) : ?>
			...
			<div id="template-notices" role="alert" aria-atomic="true">
				<?php if ( bp_registration_needs_activation() ) : ?>
					<p><?php _e( 'You have successfully created your account! To begin using this site you will need to activate your account via the email we have just sent to your address.', 'kids-world' ); ?></p>
				<?php else : ?>
					<p><?php _e( 'You have successfully created your account! Please log in using the username and password you have just created.', 'kids-world' ); ?></p>
				<?php endif; ?>
			</div>
