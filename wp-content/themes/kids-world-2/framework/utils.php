...
function kidsworld_default_option() {
	...
	$pageoptions = array(
		...
		'post-archives-readmore' => '[dt_sc_button title="Read More" target="_blank" /]'
	);
	...
	$fonts = array(
		...
		'h5-font' => 'Roboto',
		'h6-font' => 'Roboto',
		'font-style' => array( '100', '300', '400', '400italic', '500', '600', '700', '800' ),
 ...
 * --------------------------------------------------------------------------- */
function kidsworld_is_plugin_active($plugin) {
	return in_array( $plugin, (array) get_option( 'active_plugins', array() ) ) || kidsworld_is_plugin_active_for_network( $plugin );
}

function kidsworld_is_plugin_active_for_network( $plugin ) {
	if ( !is_multisite() )
		return false;
