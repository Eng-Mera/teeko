 ...
 * Theme support
 * --------------------------------------------------------------------------- */
if (!function_exists('kidsworld_features')) {

	function kidsworld_features() {
		...
		if ( function_exists( 'add_theme_support' ) ) {
			...
			add_image_size( 'kidsworld-blog-thumb', 150, 120, true  ); 	// blog - list
			add_image_size( 'kidsworld-event-list', 420, 336, true  ); 	// event-calendar - list
			add_image_size( 'kidsworld-event-single-type1', 773, 520, true  ); // event-calendar - single
			add_image_size( 'kidsworld-event-single-type4', 570, 460, true  ); // event-calendar - single
			add_image_size( 'kidsworld-event-single-type5', 746, 770, true  ); // event-calendar - single
			add_image_size( 'kidsworld-event-list2', 420, 275, true  ); 	// event-calendar - shortcode list

			if(class_exists('DTRestaurantAddon')):
				add_image_size( 'kidsworld-menu-type2', 130, 130, true  ); // menu type2
				add_image_size( 'kidsworld-chef-type1', 420, 383, true  ); // chef type1
				add_image_size( 'kidsworld-chef-type2', 420, 420, true  ); // chef type2
			endif;

			if(class_exists('DTRoomAddon'))
				add_image_size( 'kidsworld-room-type', 400, 297, true  ); // room type
				
			if(class_exists('DTEventAddon')):
				add_image_size( 'kidsworld-events-grid', 375, 375, true  ); // events grid
				add_image_size( 'kidsworld-events-list', 420, 285, true  ); // events list
			endif;
			
			if(class_exists('DTProgramAddon')):
				add_image_size( 'kidsworld-training-list2', 280, 311, true  ); // training list2
			endif;
		}
	...
	add_action('after_setup_theme', 'kidsworld_features');
}

/* ---------------------------------------------------------------------------
 * When theme activation, update default option
 * --------------------------------------------------------------------------- */
if (!function_exists('kidsworld_activation_function')) {

	// check activate theme
	function kidsworld_activation_function($oldname, $oldtheme=false) {
		if(!in_array($oldname, array('kids-world', 'KidsWorld Child'))) {
			update_option(KIDSWORLD_SETTINGS, kidsworld_default_option());
		}
	}
	add_action("after_switch_theme", "kidsworld_activation_function", 10 , 2);
}

/* ---------------------------------------------------------------------------
 *	Under Construction
 * --------------------------------------------------------------------------- */
if( ! function_exists('kidsworld_under_construction') ){
	function kidsworld_under_construction(){
...
if( kidsworld_option('pageoptions','enable-comingsoon') ):
	add_action('template_redirect', 'kidsworld_under_construction', 30);

	// getting shortcode css ----------------------
	add_action('wp_enqueue_scripts', 'kidsworld_rand_css', 101);
	function kidsworld_rand_css() {
		$id = kidsworld_option('pageoptions','comingsoon-pageid');
		if ( $id ) {
			$shortcodes_custom_css = get_post_meta( $id, '_wpb_shortcodes_custom_css', true );
			if ( ! empty( $shortcodes_custom_css ) ) {
				
				wp_register_style( 'vc_shortcodes-custom-'.$id, '', false, KIDSWORLD_THEME_VERSION, 'all' );	
 ...
 * --------------------------------------------------------------------------- */
if( !function_exists('kidsworld_wp_list_categories') ){
	function kidsworld_wp_list_categories( $output ){
	...
	add_filter('wp_list_categories', 'kidsworld_wp_list_categories');
}

/* ---------------------------------------------------------------------------
 * Filter to modify default list archive widget view
 * --------------------------------------------------------------------------- */
if( !function_exists('kidsworld_wp_list_archive') ){
	function kidsworld_wp_list_archive( $link_html,$url, $text, $format, $before, $after ) {
	...
	add_filter('get_archives_link', 'kidsworld_wp_list_archive', 10, 6);	
}

/* ---------------------------------------------------------------------------
 * Filter to execute shortcode inside contact form7
 * --------------------------------------------------------------------------- */
if( !function_exists('kidsworld_wpcf7_form_elements') ){
	function kidsworld_wpcf7_form_elements($form) {
		$form = do_shortcode( $form );
		return $form;
	}
	add_filter('wpcf7_form_elements', 'kidsworld_wpcf7_form_elements');
}

/* ---------------------------------------------------------------------------
 * Get Current Page or Page ID
 * --------------------------------------------------------------------------- */
function kidsworld_ID(){
	global $post;
	$postID = false;
 ...
 * --------------------------------------------------------------------------- */
function kidsworld_slider( $id = false ){
	...
	if( $id || is_home() || get_post_type() == 'page' ) {
		...
		elseif( is_page() ) $id = kidsworld_ID();
		...
		if( is_array( $slider_key) && array_key_exists('slider_type', $slider_key) ) {
			...
			if( ( $slider_key['slider_type'] == 'revolutionslider' ) && ( array_key_exists( 'revolutionslider_id', $slider_key ) ) && ( array_key_exists( 'show_slider', $slider_key ) ) ) {
				...
				$slider = '<div id="slider"><div class="dt-sc-main-slider" id="dt-sc-custom-slider">';
					$slider .= kidsworld_wp_kses(do_shortcode( $slider_key['customslider_sc'] ));
				$slider .= '</div></div>';
			}
		}
	}
 ...
 * --------------------------------------------------------------------------- */
function kidsworld_pagination( $query = false, $load_more = false ){
	global $wp_query;
	...
	$end_size = 1;
	$mid_size = 2;
	$show_all = kidsworld_option('general','showall-pagination');
	$dots = false;
		...
		if( $load_more ){
			...
			if( $paged < $total ){
				$output .= '<div class="column one pager_wrapper pager_lm">';
					$output .= '<a class="pager_load_more button button_js" href="'. get_pagenum_link( $next ) .'">';
						...
						$output .= '<span class="button_label">'. esc_html__('Load more', 'kids-world') .'</span>';
					$output .= '</a>';
				$output .= '</div>';
			}
			...
			$output .= '<div class="column one pager_wrapper">';
				...
				$args = array(
					...
					'mid_size'           => $mid_size,
					'prev_next'          => true,
					'prev_text'          => '<i class="fa fa-angle-double-left"></i>'.esc_html__('Prev', 'kids-world'),
					'next_text'          => esc_html__('Next', 'kids-world').'<i class="fa fa-angle-double-right"></i>',
					'type'               => 'list'
				);
 ...
 * Page Title
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'kidsworld_page_title' ) ){
	function kidsworld_page_title( $echo = false ){
		...
		if( function_exists('tribe_is_month') && ( tribe_is_event_query() || tribe_is_month() || tribe_is_event() || tribe_is_day() || tribe_is_venue() ) ){
			...
			$title = get_the_title( kidsworld_ID() );
 ...
 * Breadcrumbs
 * --------------------------------------------------------------------------- */
function kidsworld_breadcrumbs(){
	global $post;
	
	$homeLink = esc_url( home_url('/') );
	$separator = '<span class="'.kidsworld_option('layout','breadcrumb-delimiter').'"></span>';
	...
	if( function_exists('is_bbpress') && is_bbpress() ){
		bbp_breadcrumb( array(
			...
			'after' 		=> '</div>',
			'sep' 			=> '<span class="'.kidsworld_option('layout','breadcrumb-delimiter').'"></span>',
			'crumb_before' 	=> '',
			'crumb_after' 	=> '',
			'home_text' 	=> esc_html__('Home','kids-world'),
		) );
		return true;
	}
	...
	$breadcrumbs[] =  '<a href="'. $homeLink .'">'. esc_html__('Home','kids-world') .'</a>';
	...
	if( function_exists('tribe_is_month') && ( tribe_is_event_query() || tribe_is_month() || tribe_is_event() || tribe_is_day() || tribe_is_venue() ) ) {
		...
		$breadcrumbs[] = '<a href="">'.esc_html__('Search', 'kids-world').'</a>';

	} elseif( is_single() && ! is_attachment() ){

		if( get_post_type() == 'dt_portfolios' ){

			$cat = get_the_term_list(kidsworld_ID(), 'portfolio_entries', '', '$$$', '');
			...
			$cat = get_the_term_list(kidsworld_ID(), 'product_cat', '', '$$$', '');
			$cats = array_filter(explode('$$$', $cat));
			if (!empty($cats))
				$breadcrumbs[] = $cats[0];
		...
		$breadcrumbs[] = '<span class="current">'. get_the_title( kidsworld_ID() ) .'</span>';
 ...
 * Breadcrumb Section
 * --------------------------------------------------------------------------- */
function kidsworld_breadcrumb_section($id, $post_type ) {

	$bcrumb = kidsworld_option('layout','show-breadcrumb');
	if( isset($bcrumb) && $bcrumb == 'true' ) :
		...
		if(!empty($bg) || !empty($co)) :
			...
			$color = !empty($settings['sub-title-bg-color']) ? kidsworld_hex2rgb($settings['sub-title-bg-color']) : '';
		else:
			$bg = kidsworld_option('layout','sub-title-bg');
			$opacity = kidsworld_option('layout','sub-title-opacity');
			$opacity = !empty($opacity) ? $opacity : '1';
			$position = kidsworld_option('layout','sub-title-bg-position');
			$position = !empty($position) ? $position : 'center center';
			$repeat = kidsworld_option('layout','sub-title-bg-repeat');
			$repeat = !empty($repeat) ? $repeat : 'repeat';
			$color = kidsworld_option('layout','sub-title-bg-color');
			$color = !empty($color) ? kidsworld_hex2rgb($color) : '';
		endif;

		

		$subtitle_parallax = kidsworld_option('layout','sub-title-parallax');
		if(isset($subtitle_parallax) && $subtitle_parallax == 'true') :
			$style = !empty($bg) ? "background:url($bg) repeat scroll 50% 43px;" : "";
			$parallax_class = 'dt-sc-subtitle-parallax';
		else:
			$style = !empty($bg) ? "background:url($bg) $position $repeat;" : "";
			$parallax_class = '';
		endif;

		$style .= !empty($color) ? "background:rgba(  $color[0] ,  $color[1],  $color[2], {$opacity});" : "";

		$bstyle = kidsworld_option('layout','breadcrumb-style');
		$bstyle .= kidsworld_option('layout','breadcrumb-darkbg') ? ' dt-sc-dark-bg ' : '';

		echo '<section class="main-title-section-wrapper '.esc_attr($bstyle).' '.esc_attr($parallax_class).
...
function kidsworld_breadcrumb_section_with_class ( $title , $class ){
	$bcrumb = kidsworld_option('layout','show-breadcrumb');
	if( isset($bcrumb) && $bcrumb == 'true' ) :
...
function kidsworld_events_title() {
	...
	if( tribe_is_month() && !is_tax() ) { 
		$title = sprintf( esc_html__( 'Events for %s', 'kids-world' ), date_i18n( 'F Y', strtotime( tribe_get_month_view_date() ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_week() )  {
		$title = sprintf( esc_html__('Events for week of %s', 'kids-world'), date_i18n( $date_format, strtotime( tribe_get_first_week_day($wp_query->get('start_date') ) ) ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && tribe_is_day() ) {
		$title = esc_html__( 'Events for', 'kids-world' ) . ' ' . date_i18n( $date_format, strtotime( $wp_query->get('start_date') ) );
	} elseif( class_exists('Tribe__Events__Pro__Main') && (tribe_is_map() || tribe_is_photo()) ) {
		if( tribe_is_past() ) {
			$title = esc_html__( 'Past Events', 'kids-world' );
		} else {
			$title = esc_html__( 'Upcoming Events', 'kids-world' );
		}
		...
		$title = esc_html__('Tag Archives','kids-world');
	}
	return $title;
}
 ...
 * Excerpt
 * --------------------------------------------------------------------------- */
function kidsworld_excerpt($limit = NULL) {
 ...
 * --------------------------------------------------------------------------- */
function kidsworld_wp_kses($content) {
 ...
 * --------------------------------------------------------------------------- */
if(!function_exists('kidsworld_hex2rgb')) {

	function kidsworld_hex2rgb($hex) {
		$hex = str_replace ( "#", "", $hex );
 ...
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'kidsworld_opts_get' ) ) {

	function kidsworld_opts_get( $opt_name, $default = null ){

		// getting option
		$opts = get_option ( KIDSWORLD_SETTINGS );
		$opts = is_array( $opts ) ? $opts : array();

		$value = kidsworld_array_search($opts, $opt_name);
		if( ! empty( $value ) ){
			return $value;
		} else {
			return $default;
 ...
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'kidsworld_opts_show' ) ) {

	function kidsworld_opts_show( $opt_name, $default = null ){
 ...
 * --------------------------------------------------------------------------- */
function kidsworld_array_search(array $array, $search) {
	$keys = false;
	...
	foreach ($array as $key => $value) {
		if (is_array($array[$key])) {
			if (array_key_exists($search, $array[$key])) {
			    ...
			    $keys = kidsworld_array_search($array[$key], $search);
			}
		}
	}
	return $keys;
 ...
 * Theme Header Logo
 * --------------------------------------------------------------------------- */
function kidsworld_header_logo() {
	echo '<div id="logo">';
		if( kidsworld_option('layout', 'logo') ):
			$url = kidsworld_option('layout', 'logo-url');
			$url = !empty( $url ) ? $url : KIDSWORLD_THEME_URI . "/images/logo.png";

			$darkbg_url = kidsworld_option('layout','darkbg-logo-url');
			$darkbg_url = !empty($darkbg_url) ? $darkbg_url : KIDSWORLD_THEME_URI ."/images/light-logo.png"; ?>
 ...
 * Theme Comment Style
 * --------------------------------------------------------------------------- */
function kidsworld_comment_style( $comment, $args, $depth ) {
	...
	case 'trackback':
		...
		echo "<p>";
		esc_html_e('Pingback:', 'kids-world');
		comment_author_link();
		edit_comment_link(esc_html__('Edit', 'kids-world'), ' ', '');
		echo "</p>";
		break;
	...
	case '':
		...
		echo 		'<h3>'.get_comment_author_link().'</h3>';
		echo '		<span class="commentmetadata"> / '.get_comment_date('d M Y').'</span>';
		echo '	</div>';
		echo '  <div class="comment-body">';
					comment_text();
					if ($comment->comment_approved == '0') :
						esc_html_e('Your comment is awaiting moderation.', 'kids-world');
					endif;
					edit_comment_link(esc_html__('Edit', 'kids-world'));
		echo '	</div>';
		echo '	<div class="reply">';
		echo 		comment_reply_link(array_merge($args, array('reply_text' => esc_html__('Reply', 'kids-world'), 'depth' => $depth, 'max_depth' => $args['max_depth'])));
		echo '	</div>';
		echo '</section>';
 ...
 * Theme show sidebar
 * --------------------------------------------------------------------------- */
function kidsworld_show_sidebar( $type, $id, $position = 'right' ) {
 ...
 * --------------------------------------------------------------------------- */
function kidsworld_global_variables($variable = '') {

	global $woocommerce, $product, $woocommerce_loop, $post, $wp_query, $pagenow, $kidsworld_like_love, $wpsl_settings, $wpsl;
	...
	switch($variable) {
		...
		case 'pagenow':
			return $pagenow;
		break;
		case 'kidsworld_like_love':
			return $kidsworld_like_love;
		break;
		case "wpsl_settings":
...
* break oembeds.
*/
function kidsworld_undo_recurrence_oembed_logic() {
...
add_action( 'init', 'kidsworld_undo_recurrence_oembed_logic' );


/* ---------------------------------------------------------------------------
 * Get header type
 * --------------------------------------------------------------------------- */
if (!function_exists('kidsworld_get_header_type')) {

	function kidsworld_get_header_type() {
		...
		if( isset($post) && is_page($post->ID) ) :
			...
			$htype = (isset($tpl_default_settings['header-type']) && $tpl_default_settings['header-type'] != '') ? $tpl_default_settings['header-type'] : kidsworld_option('layout','header-type');
		else:
